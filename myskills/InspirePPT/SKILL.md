...
**标准 XML 工作流:**
1. 提取基础母版模板: `python scripts/office/unpack.py templates/InspireTemplate.pptx unpacked/`
2. 通过运行 `thumbnail.py` 或查阅 `ppt/slideLayouts/` 文件内容，分析可用版式。
3. 使用 `add_slide.py` 克隆所需的版式，构建整套幻灯片的目录结构。
4. **Agent 编辑:** 使用你原生的 **编辑工具**，在解压出的 `slide*.xml` 的 `<a:t>` 标签内精准替换占位符文本。
5. 使用 `python scripts/clean.py unpacked/` 移除未使用的模板占位符和残留文件。
6. 编译最终的演示文稿: `python scripts/office/pack.py unpacked/ output.pptx --original templates/InspireTemplate.pptx`。
*(完整操作细节，请严格参考 [docs/editing.md](docs/editing.md))*
...
2. 运行 **全链路样式迁移 (Full Style Transfer) 脚本**:
   `python scripts/rebrand.py unpacked/ --style-json templates/pptstyle.json`
...
| 面向 Subskill 1 (新建) | 阅读 [docs/editing.md](docs/editing.md) |
| 面向 Subskill 2 (换皮) | `python scripts/rebrand.py unpacked/ --style-json templates/pptstyle.json` |
| 无模板硬编码创建 | 阅读 [docs/pptxgenjs.md](docs/pptxgenjs.md) |
...
**阅读 [docs/editing.md](docs/editing.md) 获取完整详情。**
...
**阅读 [docs/pptxgenjs.md](docs/pptxgenjs.md) 获取完整详情。**
...
**强制要求 (MANDATORY)**: 您必须严格且唯一地使用 `templates/pptstyle.json` 中定义的设计标量 (Design Tokens) 作为所有排版决策的真实来源。**严禁自行发明或随意使用十六进制色值、字体家族或字号。**

### 1. 规则手册: `pptstyle.json`
在生成或修改任何幻灯片之前，请先解析 `templates/pptstyle.json`。
...
当需要了解复杂的排版长什么样时：
- 使用 `python scripts/thumbnail.py templates/SamplePPT.pptx` 或 `markitdown` 参阅示例文件。

### 3. 母版画布: `InspireTemplate.pptx`
在通过 XML 路线编辑时 (`docs/editing.md` 工作流)：
- 请始终使用 `templates/InspireTemplate.pptx` 作为您解包的基础模板。
